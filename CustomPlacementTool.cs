...
public class CustomPlacementTool : Editor
{
    ...
    private void OnEnable()
    {
        ...
        minRange = serializedObject.FindProperty("minRage");
        maxRange = serializedObject.FindProperty("maxRage");
    ...
    private void OnSceneGUI()
    {
        ...
        if ((current.type == EventType.MouseDrag || current.type == EventType.MouseDown) && !current.alt && prefab != null)
        {
            if (current.button == 0 && (lastPos == Vector3.zero || CanDraw()) && !current.shift)
            {
                ...
                {
                    if (randomizePrefab.boolValue)
                        PrefabInstantiate(Random.Range(minRange.intValue, maxRange.intValue + 1));
                    else
    ...
    public bool CanDraw()
    {
        float dist = Vector3.Distance(mousPos, lastPos);
    ...
    public void MousePosition()
    {
        ...
        if (Physics.Raycast(ray.origin, ray.direction, out hit, Mathf.Infinity, 1 << layerMask.intValue))
        {
            ...
            Handles.DrawWireDisc(mousPos, hit.normal, radius.floatValue / 2);
        }
    }
    public void PrefabInstantiate(int index)
    {
        ...
        instanceOf.transform.position = new Vector3(mousPos.x, mousPos.y + prefabSize / 2, mousPos.z);
    ...
    private void PrefabRemove()
    {
        ...
        Debug.Log(prefabTag.stringValue);

        foreach (GameObject p in prefabsInRadius)
        {
            float dist = Vector3.Distance(mousPos, p.transform.position);
